export type BookItem = {
  id: string
  title: string
  author: string
  publisher: string
  publisherMark: string
  publisherMarkExt?: 'svg' | 'png'
  spineColor: string
  textColor: string
  pages: number
  coverSrc?: string
  /** BCP-47 tag when the title is not English. */
  lang?: string
}

/** Same shelf as the home page bookshelf strip. */
export const bookshelf: BookItem[] = [
  {
    id: 'construction-of-social-reality',
    title: 'The Construction of Social Reality',
    author: 'John R. Searle',
    publisher: 'Free Press',
    publisherMark: 'simon-schuster',
    publisherMarkExt: 'png',
    spineColor: '#02061f',
    textColor: '#f0f4fb',
    pages: 256,
    coverSrc: 'https://covers.openlibrary.org/b/isbn/9780684831794-M.jpg',
  },
  {
    id: 'design-of-everyday-things',
    title: 'The Design of Everyday Things',
    author: 'Don Norman',
    publisher: 'Basic Books',
    publisherMark: 'basic-books',
    spineColor: '#f0de2f',
    textColor: '#141414',
    pages: 368,
    coverSrc: 'https://covers.openlibrary.org/b/isbn/9780465050659-M.jpg',
  },
  {
    id: 'dying-for-ideas',
    title: 'Dying for Ideas',
    author: 'Costica Bradatan',
    publisher: 'Bloomsbury Academic',
    publisherMark: 'bloomsbury',
    spineColor: '#3d2528',
    textColor: '#f4ebe0',
    pages: 256,
    coverSrc:
      'https://books.google.com/books/content?id=jOxJBgAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=nocurl&source=gbs_api',
  },
  {
    id: 'against-the-machine',
    title: 'Against the Machine',
    author: 'Paul Kingsnorth',
    publisher: 'Penguin',
    publisherMark: 'penguin-random-house',
    publisherMarkExt: 'png',
    spineColor: '#1f5d2d',
    textColor: '#f0f7ef',
    pages: 368,
    coverSrc:
      'https://books.google.com/books/content?id=dbod0QEACAAJ&printsec=frontcover&img=1&zoom=2&edge=nocurl&source=gbs_api',
  },
  {
    id: 'cultural-ideologies-romania',
    title: 'Ideologies in Contemporary Romania',
    author: 'Claude Karnoouh',
    publisher: 'Penguin',
    publisherMark: 'penguin-random-house',
    publisherMarkExt: 'png',
    spineColor: '#dde0e3',
    textColor: '#2b3039',
    pages: 304,
  },
  {
    id: 'simple-genius',
    title: 'Simple Genius',
    author: 'David Baldacci',
    publisher: 'Grand Central Publishing',
    publisherMark: 'grand-central',
    spineColor: '#eceeef',
    textColor: '#121722',
    pages: 420,
    coverSrc: 'https://covers.openlibrary.org/b/isbn/9780446581738-M.jpg',
  },
  {
    id: 'atlas-shrugged',
    title: 'Atlas Shrugged',
    author: 'Ayn Rand',
    publisher: 'Dutton',
    publisherMark: 'penguin-random-house',
    publisherMarkExt: 'png',
    spineColor: '#f4f4f5',
    textColor: '#1d2126',
    pages: 1184,
    coverSrc: 'https://covers.openlibrary.org/b/isbn/0679417397-M.jpg',
  },
  {
    id: 'the-black-swan',
    title: 'The Black Swan',
    author: 'Nassim Nicholas Taleb',
    publisher: 'Random House',
    publisherMark: 'penguin-random-house',
    publisherMarkExt: 'png',
    spineColor: '#171717',
    textColor: '#f5f5f5',
    pages: 423,
    coverSrc: 'https://covers.openlibrary.org/b/isbn/0452284236-M.jpg',
  },
  {
    id: 'meditations',
    title: 'Meditations',
    author: 'Marcus Aurelius',
    publisher: 'Modern Library',
    publisherMark: 'penguin-random-house',
    publisherMarkExt: 'png',
    spineColor: '#4f3b2b',
    textColor: '#f9f2e8',
    pages: 273,
    coverSrc:
      'https://books.google.com/books/content?id=brSidvTKfcQC&printsec=frontcover&img=1&zoom=1&edge=nocurl&source=gbs_api',
  },
  {
    id: 'manufacturing-consent',
    title: 'Manufacturing Consent',
    author: 'Noam Chomsky',
    publisher: 'Pantheon Books',
    publisherMark: 'pantheon',
    spineColor: '#7b1e1f',
    textColor: '#fff6f4',
    pages: 412,
    coverSrc: 'https://covers.openlibrary.org/b/id/7900362-M.jpg',
  },
  {
    id: 'the-society-of-the-spectacle',
    title: 'The Society of the Spectacle',
    author: 'Guy Debord',
    publisher: 'Black & Red',
    publisherMark: 'black-red',
    spineColor: '#2f3a56',
    textColor: '#eff3ff',
    pages: 192,
    coverSrc: 'https://covers.openlibrary.org/b/isbn/0934868077-M.jpg',
  },
  {
    id: 'the-brothers-karamazov',
    title: 'The Brothers Karamazov',
    author: 'Fyodor Dostoevsky',
    publisher: 'Penguin Classics',
    publisherMark: 'penguin-random-house',
    publisherMarkExt: 'png',
    spineColor: '#1f2a29',
    textColor: '#eaf7f5',
    pages: 856,
    coverSrc:
      'https://books.google.com/books/content?id=VhfYK9jgYAYC&printsec=frontcover&img=1&zoom=1&edge=nocurl&source=gbs_api',
  },
  {
    id: 'capitalist-realism',
    title: 'Capitalist Realism',
    author: 'Mark Fisher',
    publisher: 'Zero Books',
    publisherMark: 'zero-books',
    spineColor: '#313131',
    textColor: '#f2f2f2',
    pages: 88,
    coverSrc:
      'https://books.google.com/books/content?id=QSiUEAAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=nocurl&source=gbs_api',
  },
  {
    id: 'letters-to-a-young-poet',
    title: 'Letters to a Young Poet',
    author: 'Rainer Maria Rilke',
    publisher: 'Penguin Classics',
    publisherMark: 'penguin-random-house',
    publisherMarkExt: 'png',
    spineColor: '#6e4a68',
    textColor: '#f7ecf6',
    pages: 104,
    coverSrc:
      'https://books.google.com/books/content?id=lwHgAgAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=nocurl&source=gbs_api',
  },
  {
    id: 'critique-of-pure-reason',
    title: 'Critique of Pure Reason',
    author: 'Immanuel Kant',
    publisher: 'Open Road Media',
    publisherMark: 'open-road',
    spineColor: '#27374d',
    textColor: '#f2f6fb',
    pages: 798,
    coverSrc: 'https://openroadmedia.com/ebook/the-critique-of-pure-reason/9781504004626',
  },
  {
    id: 'the-stranger',
    title: 'The Stranger',
    author: 'Albert Camus',
    publisher: 'Gallimard',
    publisherMark: 'gallimard',
    spineColor: '#b73a2f',
    textColor: '#fff8ea',
    pages: 159,
    coverSrc:
      'https://upload.wikimedia.org/wikipedia/commons/9/97/L%27%C3%89tranger_-_Albert_Camus.jpg',
  },
  {
    id: 'anna-karenina',
    title: 'Anna Karenina',
    author: 'Leo Tolstoy',
    publisher: 'Penguin Classics',
    publisherMark: 'penguin-random-house',
    publisherMarkExt: 'png',
    spineColor: '#2d3a2f',
    textColor: '#edf3ea',
    pages: 864,
    coverSrc: 'https://prodimage.images-bn.com/pimages/9780140449174_p0_v2_s600x595.jpg',
  },
  {
    id: 'ek-chuhe-ki-maut',
    title: 'एक चूहे की मौत',
    author: 'Badiuzzaman',
    publisher: 'Rajkamal Prakashan',
    publisherMark: 'rajkamal',
    spineColor: '#6f2b2b',
    textColor: '#fff3e6',
    pages: 176,
    lang: 'hi',
  },
  {
    id: 'suraj-ka-satwan-ghoda',
    title: 'सूरज का सातवाँ घोड़ा',
    author: 'Dharamvir Bharati',
    publisher: 'Bharatiya Jnanpith',
    publisherMark: 'jnanpith',
    publisherMarkExt: 'png',
    spineColor: '#d38a27',
    textColor: '#21160a',
    pages: 112,
    lang: 'hi',
  },
  {
    id: 'tamas',
    title: 'तमस',
    author: 'Bhisham Sahni',
    publisher: 'Rajkamal Prakashan',
    publisherMark: 'rajkamal',
    spineColor: '#29231f',
    textColor: '#f5e8d4',
    pages: 352,
    lang: 'hi',
  },
  {
    id: 'raag-darbari',
    title: 'राग दरबारी',
    author: 'Shrilal Shukla',
    publisher: 'Rajkamal Prakashan',
    publisherMark: 'rajkamal',
    spineColor: '#52633f',
    textColor: '#f3f0d8',
    pages: 384,
    lang: 'hi',
  },
  {
    id: 've-din',
    title: 'वे दिन',
    author: 'Nirmal Verma',
    publisher: 'Rajkamal Prakashan',
    publisherMark: 'rajkamal',
    spineColor: '#7b6b83',
    textColor: '#faf1f5',
    pages: 192,
    lang: 'hi',
  },
  {
    id: 'the-metamorphosis',
    title: 'The Metamorphosis',
    author: 'Franz Kafka',
    publisher: 'Penguin Classics',
    publisherMark: 'penguin-random-house',
    publisherMarkExt: 'png',
    spineColor: '#4a4b3f',
    textColor: '#f0eee4',
    pages: 128,
  },
  {
    id: 'the-overcoat',
    title: 'The Overcoat',
    author: 'Nikolai Gogol',
    publisher: 'Penguin Classics',
    publisherMark: 'penguin-random-house',
    publisherMarkExt: 'png',
    spineColor: '#35505f',
    textColor: '#edf6f8',
    pages: 48,
  },
  {
    id: 'notes-from-underground',
    title: 'Notes from Underground',
    author: 'Fyodor Dostoevsky',
    publisher: 'Penguin Classics',
    publisherMark: 'penguin-random-house',
    publisherMarkExt: 'png',
    spineColor: '#5b3038',
    textColor: '#f8eaec',
    pages: 160,
  },
]
